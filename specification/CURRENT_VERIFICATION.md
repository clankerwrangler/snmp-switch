...
The 51-configuration normal catalog is supported by separate completed checks,
not one combined 51-config run. All passing graphs below finished with zero
...
| Group migration/conversion | PASS; full temporal check; 25.631 s | 9,328 / 6,996 |
| Group form submissions | PASS; full temporal check; 7.626 s | 12,672 / 10,560 |
| Group access/queued SET | PASS; full temporal check; 12.688 s | 36,932 / 31,656 |
...
counts. The latest group migration result includes selected-target inline
preservation. Group form/queue results retain the earlier stock-USM run's actual
whole-file hashes: their unchanged action/property dependency slices were
reviewed, but those two checks were not rerun after the unrelated inline change.
The original lower-level-USM assumption was superseded after a real runtime probe
failed; its earlier conditional model results do not establish weaker-level
management support.
...
The six reusable controls in `tests/check_mutations.py` run against current
sources. Other historical control definitions and exact inputs are in Git.
...
```sh
python3 run_tlc.py --java /path/to/java --jar /path/to/tla2tools.jar --models GroupMigrationConversion GroupAccessForms GroupQueuedSet --timeout 120 --batch-timeout 360 --heap 1g --max-state-mib 2048 --min-free-mib 20480 --output logs/groups-1
```
...
Generators reproduce four scenario modules and 32 configs. Run them in a
throwaway checkout and compare the generated bytes before accepting edits:

```sh
python3 make_configs.py
python3 tests/make_scenarios.py
python3 tests/make_access_scenarios.py
python3 tests/make_management_scenarios.py
```
