...
VARIABLES store, work, result, seen, scenario, workKind, deliveryStage, pc, anchor
StorageVars == <<store,work,result,seen,scenario,workKind,deliveryStage,pc,anchor>>
...
StorageInit == BaseInit /\ scenario = "graph" /\ workKind = "set" /\ deliveryStage = "afterRestart"
...
CommitState(value, outcome) ==
    CASE outcome = "success" -> [store EXCEPT !.published = value, !.durable = value,
      ...
      [] outcome = "commitUnknown" -> [store EXCEPT !.durable = value, !.health = "commitUnknown",
                                      !.transaction = NoVersion]
\* Every persistent API/SET transaction uses the same Store/Engine health gate.
Attempt(kind, field, outcome) ==
    /\ kind \in {"api","set"} /\ field \in Fields /\ outcome \in Outcomes
    /\ (IF CommitAllowed
       THEN /\ store' = CommitState(Edit(store.published,field),outcome)
...
Handle(outcome) == /\ work # NoWork /\ outcome \in Outcomes
    /\ (IF CurrentWork
       THEN IF work.kind = "notify"
            ...
            ELSE IF work.kind = "background"
                 ...
                 ELSE /\ store' = CommitState(Edit(store.published,"secondary"),outcome)
                      /\ result' = OutcomeResult(outcome)
       ELSE /\ UNCHANGED store /\ result' = Result("workDropped",0))
    /\ work' = NoWork /\ UNCHANGED seen
HandleWork == \E outcome \in Outcomes : Handle(outcome)
...
Open(load) == /\ ~store.open /\ load \in {"valid","unavailable","invalid"}
    ...
    /\ UNCHANGED <<work,seen>>
KeepFixture == UNCHANGED <<scenario,workKind,deliveryStage,pc,anchor>>
StorageNext ==
    /\ ((\E k \in {"api","set"}, f \in Fields, o \in Outcomes : Attempt(k,f,o))
...
NormalCapabilities == Gate =>
    /\ ENABLED Attempt("api","secondary","success")
    /\ ENABLED Attempt("set","secondary","success") /\ ENABLED Background /\ ENABLED Send
...
FailedRollbackOutcome == [][result'.status = "undoFailed" =>
    ...
    /\ store'.durable = store.durable /\ store'.published = store.published /\ result'.index = 0]_StorageVars
UnknownCommitOutcome == [][result'.status = "dropped" =>
    /\ store'.health = "commitUnknown" /\ store'.transaction = NoVersion
    /\ store'.published = store.published /\ result'.index = 0]_StorageVars
...
RecoveryAction == CASE pc = 0 -> Queue(workKind)
    [] pc = 1 -> Attempt("set","primary",scenario)
    [] pc = 2 -> ReadConfirmed
    [] pc = 3 -> InspectStorage
    [] pc = 4 -> Attempt("api","secondary","success")
    [] pc = 5 -> Attempt("set","secondary","success")
    [] pc = 6 -> Background
    [] pc = 7 -> Send
    [] pc = 8 -> IF deliveryStage = "faulted" THEN Handle("success") ELSE Wait
    ...
    [] pc = 14 -> Close
    [] pc = 15 -> Open("valid")
    [] pc = 16 -> IF work = NoWork THEN Wait ELSE Handle("success")
    [] pc = 17 -> Queue(workKind)
    [] pc = 18 -> Handle("success")
    [] pc = 19 -> Attempt("api","secondary","success")
    [] pc = 20 -> Send
    [] pc = 21 -> ReadConfirmed
RecoveryNext == IF pc = 22 THEN UNCHANGED StorageVars
    ELSE /\ RecoveryAction /\ pc' = pc + 1
         ...
         /\ UNCHANGED <<scenario,workKind,deliveryStage>>
...
RecoveryAssertions ==
    /\ (pc = 2 => result = OutcomeResult(scenario))
