...
FormFixtures == {[kind |-> "form",owner |-> owner,name |-> name,start |-> start,finish |-> finish,
    nets |-> nets,key |-> key,on |-> on,missing |-> missing] :
    owner \in {"community","group"},name \in FormNames,start \in Modes,
    finish \in (IF name = "transition" THEN Modes ELSE {start}),nets \in Filters,
    key \in Secrets,on \in BOOLEAN,missing \in BOOLEAN}
FormStart(f) ==
