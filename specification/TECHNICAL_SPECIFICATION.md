...
SNMPv2c communities own their incoming read/write policy. SNMPv3 users own USM identity, keys, global enablement, and a configured protection profile; one optional `group_id` selects their shared incoming policy. Groups own a minimum protection level and independent read/write permissions, OID views, and optional source CIDRs. There are no per-user permission overrides. The groups are application-owned VACM-style policy, not a native VACM-table mirror or full VACM instrumentation. The existing internal notification mapping remains separate. [R8, R9, R10]

Stock PySNMP 7.1.29 accepts normal v3 management at the user's configured `noAuthNoPriv`, `authNoPriv`, or `authPriv` profile. A lower group minimum does not relax that protection requirement. After USM accepts the message, the application checks the group's minimum and policy; it adds no independent user floor. A user whose profile is below the selected group minimum can remain a member, but cannot use that group's incoming access. Discovery and REPORT exceptions are library behavior, not operational access at weaker protection. Authentication uses SHA-256 and privacy uses AES-128; the application does not implement cryptography.
