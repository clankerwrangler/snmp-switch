...
| BOOT-01 | Application restart restores explicit startup switch policy over the current durable physical lab and reboots operational state. |
...
On a force-authorized port, an untagged source uses the running PVID and an explicitly tagged source uses its configured VID. Learning requires an up port and configured admission. Force-unauthorized blocks ingress without changing carrier. Auto requires a current client/shared authorization: inherited VLAN follows PVID, while an explicit grant permits untagged or matching-tag activity in its existing non-forbidden VID without rewriting configured membership. Otherwise the activity is recorded as rejected, without an FDB update. A VLAN is not created automatically by receiving simulated activity.
...
Implement SNMPv2c and SNMPv3 `GET`, `GETNEXT`, and `GETBULK`, including correct ASN.1 types, scalar suffixes, numeric OID order, exceptions, and response limits. A PDU is evaluated against one coherent state snapshot and one authorization decision. Its projection captures the published Runtime, view, and clock sample, registers complete supported-column metadata and numeric family intervals, and builds rows only when a read reaches that family. Selected ordinary cells then materialize ASN.1 values. Global successor selection crosses hidden, empty, sparse and current-VLAN families; sorted request-local TimeFilter keys preserve the requested cutoff. Diagnostic enumeration can still materialize every ordinary family. This request-local representation does not cache data or permissions across PDUs; uptime and TimeFilter wrap are sampled for each new projection. A walk consists of several requests and is not an atomic snapshot. Never expose `not-accessible` indexes as readable columns. [R4]

View includes are numeric subtree prefixes, including single roots `0`, `1`, and `2`; actual object-valued fields retain complete-OID validation. Fresh views are `all` named **iso** with prefix `1`, `internet` with `1.3.6.1`, and the unchanged `interfaces` view. Saved explicit maps, names, restrictions and credential references are retained exactly. Missing views in a non-null legacy saved configuration resolve to the old two-view map before migration, never the broader fresh default.
...
Management-uptime wrap invalidates the previous filter epoch. Rebuild conceptual current rows and their timestamps in the new epoch from running VLAN configuration; do not delete that configuration or rewind USM security state. This adapter behavior is not represented by the VLAN-keyed TLA+ projection and needs separate wire tests.
...
After the final ready claim, one synchronous interval commits, publishes, materializes the public response, and finalizes its ticket without an await. A confirmed rollback returns `commitFailed` at the first effective assignment; an actual rollback failure returns `undoFailed`, index 0. Unconfirmed commit outcome is not labeled rollback. An applied running configuration survives a subsequent response-delivery failure; SET does not save startup configuration. The required bookkeeping transaction confirms before success; a delivery failure causes no second effect or response attempt. Empty/no-op SET changes no engine state or idempotency data. Request IDs are not durable API idempotency keys. Section 9.1 defines storage-fault recovery.
...
The selected 34 definitions use IEEE8021-PAE-MIB revision `200406220000Z` and existing ifIndex values. They are a partial profile, not complete conformance or real Ethernet EAPOL transport. Polling/writing views must include the IEEE subtree. Fresh iso includes it; internet does not, and existing saved MIB-II views are not broadened.
...
Event history has five categories: System (administration and simulation controls), Connectivity (attachments and actual link transitions), Access (authentication, authorization, sessions, dynamic authorization, and RADIUS accounting), Switching (learning, aging, admission, and capacity), and SNMP (SET and trap lifecycle). Ordinary reads and unchanged per-packet MAC refreshes are not logged. The retained window is 2,000 events; it is not a complete audit archive.

A changing SNMP SET records **SNMP SET applied** and separate running-configuration effects for each changed port, switch, or VLAN resource. The existing transaction revision links these facts; it is not an authentication-attempt or actor ID. UI port edits use the same effect vocabulary. Port effects retain changed field names and only selected typed booleans/VLAN values; no arbitrary configuration-value diff is copied into history. Actual link transitions are recorded even when port traps are disabled; trap queuing still uses the existing enablement, target, credential, and capacity gates. Failed or unpublished transactions, idempotent replay, and unchanged SETs add no change effects. A committed operation does not establish response delivery. Attachments record their actual before/after port IDs within the command.

The UI shares one event renderer across history, Recent activity, and client history. It exposes recorded subjects, reasons, safe details, simulation milliseconds, and boot/import reset boundaries. Full history offers category/subject/text/outcome filters, 50-row pages, reversible adjacent-repeat grouping, and Follow independent of simulation pause. Reading older rows or opening details suspends Follow; new rows are announced without replacing the reading window. Subject values prefer context captured with the event: numeric ifIndex for **Port ID**, endpoint/resource names or labels, source MAC, and the trap destination's address and UDP port (with brackets around IPv6 addresses). Save events use the validated result; delete events retain the prior public label. Queued trap results retain the queued destination display; accounting callbacks retain their captured server label. Internal IDs still own joins and navigation, not display text. Missing historical context stays absent; exact-ID lookups from today's inventory are marked current, and unresolved subjects use removed/unavailable wording rather than raw keys. No credentials, source-bind addresses, profile contents, or arbitrary configuration values enter this display context. Known operation/session identities and distinct authentication attempts are not merged. No actor, wall-clock timestamp, or complete authentication chain is inferred.

## 8. Reboot, persistence, and scenario reset

One assembled `Runtime.cfg` drives consumers. The encrypted Store separates automatically durable lab data from explicitly saved startup switch policy. Ordinary API/SNMP edits apply running state; **Save configuration** copies its logical policy to startup only after a confirmed transaction. Physical or mixed edits must not autosave unrelated running policy.

| Lifetime | Existing fields |
|---|---|
| Durable physical lab | Switch ID, port count, base MAC, description and FDB/endpoint/source capacities; port ID, ifIndex, bridge-port, name, mode, shared partner, forced fault, speed and MTU; complete endpoints/source identities, tags, activity and copied supplicant profiles; attachments and paused mode; complete peer material/template catalog. These hardware choices describe this simulator. |
| Running/startup switch policy | Switch name, contact, location, advertised identity, legacy VLAN, aging and notification queue limit; port alias/admin/PVID/admitted/untagged/forbidden/notification/authentication policy; VLAN records; SNMP listener, views, communities, users, groups and trap targets; RADIUS Access/accounting/DAS servers, trust/secrets, NAS identity and timers, excluding the peer material/template catalog. |
| Ephemeral runtime | Learned FDB, counters, simulation time, jobs, sessions, grants, attempts, PAE observations and current-generation scheduling. |
| Independent durable bookkeeping | Administrator hash and external encryption key; USM identity/boots; monotonic revisions, bounded events/idempotency/outboxes; accounting sequence/drop state; DAS generations, replay decisions/tombstones and highwater. These are never copied or rolled back from startup. |

Existing validated complete configuration seeds both fragments atomically on upgrade. Loading alone does not write; initialization publishes the split before consumers activate and removes the old complete key in that transaction. Partial or malformed fragments and missing cross-policy references fail initialization. Retained physical IDs select saved port policy; removed ports cannot reappear, and a genuinely new ID uses existing VLAN1/force-authorized initial policy rather than an old same-index port's settings. Invalid attachments are rejected. Current APIs still have fixed interface inventory. Peer material references validate within the durable catalog; no raw startup export or new orphan store is introduced.

Management reboot and process restart compose saved switch policy onto the current lab, discarding unsaved logical edits. End live access/accounting segments, clear FDB learning, reset counters, management uptime and simulation time, invalidate pre-boot work, cancel the old notification queue with its reason, and restart source delays. Preserve endpoints, source credentials, connections, hardware/fault properties and paused/running mode. A paused restored lab relearns only after manual advancement or resume. Composition and required bookkeeping must commit before publication; a failed reboot leaves current running state intact.

Save neither revokes current authentication/work nor resets bindings. State/configuration revisions remain monotonic and do not revert to the saved startup revision. Dirty status compares running policy with startup reconciled to current physical IDs. Management reboot retains the manager session; process restart requires login using the existing administrator hash. USM boot state remains with the adapter/Store lifecycle and is committed before the restarted listener becomes available, never restored from a scenario. [R8]
...
Protected DAS replay decisions and the real-clock safety watermark survive restart and startup-policy restoration; restored sender trust gets a fresh generation when it changes. Accounting records retain captured targets/policy and original retention horizons; mismatching restored policy cancels them visibly instead of redirecting or renewing them. Live grants do not survive. Accounting and dynamic requests use the same storage-health gates as existing mutations.

A scenario reset/import applies a selected lab baseline, automatically persisting physical fields but leaving imported logical changes unsaved, clears learning, restarts simulation time and source deadlines, and uses a new simulation epoch. It preserves the live SNMP management engine and security clock. Actual topology differences can generate link events. This reset/import transaction is an application requirement not separately model-checked in this revision; `Reboot` checks the obsolete-work invalidation pattern, not import validation or crash recovery.
...
Event creation deep-copies captured details so caller or status changes cannot rewrite history. Candidate construction copies the history sequence and shares its retained read-only records; append and retention eviction affect only the private sequence. Store loading separately decodes persisted records. Other mutable Runtime fields keep independent copies, including status and outboxes. SET retains its separately copied configuration candidate. This representation preserves old Runtime/read snapshots, validation, and commit-before-publication; it does not defer persistence or acknowledge SET early.
...
| `/switch/save` | Revision-checked, confirmed running-to-startup Save; no operational reset. |
| `/switch/reboot` | Restore startup over current durable lab, discard unsaved policy and reset operational state. |
...
Return 401/403 for web authorization failures, 404 for unknown resources, 409 for stale revisions or state conflicts, 422 for invalid configuration, 429 for resource limits, and 503 before initialization completes. An unset identity is an expected setup state, not an application-wide 503; setup and simulation endpoints remain available. A malformed nonempty `identity.sys_object_id` submitted through the API returns 422. A rejected operation has no state or notification side effects. Support idempotency keys for retryable commands whose duplication would otherwise change state. Entries whose actual effect included volatile logical changes, including mixed edits and scenario imports, return stale-incarnation 409 after reboot/restart. Pure physical/Save results may replay durably; replaying a reboot key never reboots again. The existing 256-entry bound and body-mismatch check remain. State exposes only `configuration_status.unsaved` and `configuration_status.startup_revision`, not private startup fragments.
...
Provide a switch port grid, a reusable endpoint library with a source editor, a VLAN editor, distinct **SNMP**, **RADIUS**, and general **Settings** destinations, live interface/MAC address/VLAN tables, and an event history. General settings retain switch name/location/contact, base MAC, learning limits, and reboot; SNMP owns advertised identity, access, views, and notifications. Forms share the existing sparse, revision-checked commands. One whole-app **Save configuration** action shows saved/unsaved status; logical/mixed forms use **Apply**, while durable endpoint/material/template/profile edits retain **Save**. Reboot warns that unsaved switch policy is discarded while clients/connections remain. The compact **Simulation** header control reveals the current time and **Pause/Resume**, **+1s**, and **Advance…** controls. It closes on navigation; opening or closing it does not change clock mode. The port panel distinguishes admin state, carrier, forced-down state, **Port VLAN ID (PVID)** and independent **Allowed VLANs (configured)**, untagged, and forbidden sets, shared partner, attachments, and learned MAC count. Untouched Untagged VLANs preview the API PVID/untagged-membership update; a user-edited field is exact, including empty. The SNMP settings page has compact community, user, and group sections with ordinary Add/Edit/Delete controls. Community and group forms use one **Access** dropdown: **None (traps only)**, **Read**, **Read/write**, or **Write only**. Conditional fields preserve independent read/write views and networks. Mode changes retain drafts, but hidden final-inactive restriction edits are not submitted. User forms select a group alongside authentication and global enablement, and show security-level/minimum mismatches. Protocol changes require an explicit incoming-access choice without a preselected transfer. The service card shows one actual **Listener status**, its address/port, engine identity, and independent trap transport, not separate per-access status rows. The document title is **Switch Lab**. The full-width selected-port detail uses a compact link-status header, quieter interface/speed/learning metadata, grouped link and configured VLAN facts, attachments, and readable client rows. Effective authorization stays separate from configured VLAN policy and carrier. Existing source/session actions remain available; narrow layouts stack the same client fields without changing overview colors. The RADIUS page uses five always-visible cards for Access, templates, full-width certificates, accounting, and dynamic authorization. Authentication servers remain visible within Access; compact header actions open the existing forms. CA/client certificate lists show separate template and copied-source reference counts. Bounded PEM file selection or paste is a local draft until Save; imports never implicitly select trust. Saved contents are not redisplayed; sparse replacement and reference validation remain in the existing material owner. Client rows provide on-demand allowlisted **Returned RADIUS attributes** and the shared history from existing bounded events. Missing, present, and invalid returned values are distinct from effective policy. Unclassified/private attributes expose presence/count only. Background refresh preserves disclosure, focus, scroll, and selection; accounting delivery status does not determine authorization. Endpoint profiles remain in the endpoint library; there is no parallel endpoint inventory.
...
| Restart/startup Save | Core operational `Reboot`, queued-work scenarios, and StorageOutcomes/Recovery/StartupLifecycle | Exact field split, encrypted migration/Save, physical-ID reconciliation, monotonic revisions, idempotency, independent protocol durability, USM boots and relearning. |
