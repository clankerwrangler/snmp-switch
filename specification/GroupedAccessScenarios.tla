...
MigrationAction ==
    ...
    [] fixture.kind = "newUser" -> NewUser(C2,SelectedTarget,fixture.transfer,fixture.inline,fixture.persist)
...
MigrationAssertions == pc = MigrationLength =>
    ...
    [] fixture.kind = "newUser" ->
         IF fixture.transfer = "absent" /\ fixture.persist THEN
            ...
            /\ cfg.targets = (IF fixture.inline THEN [origin.targets EXCEPT ![SelectedTarget] = C2] ELSE origin.targets)
            /\ cfg.targetOn = (IF fixture.inline THEN origin.targetOn \cup {SelectedTarget} ELSE origin.targetOn)
            /\ (fixture.inline => TrapAllowed(SelectedTarget))
         ELSE cfg = origin
    [] fixture.kind = "same" ->
