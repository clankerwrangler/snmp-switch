...
| Environment | Effective identity policy |
|---|---|
| Explicit isolated-development setup | Load the development override: `1.3.6.1.4.1.32473.1`. Supply test credentials and destinations separately. |
| Public release, fresh installation | Leave `identity.sys_object_id` null. The operator supplies a full numeric OID before enabling SNMP. |
| Existing operator configuration | Preserve the saved value or intentionally unset state across restart and upgrade. |
...
The UI, API, and switch simulation start with identity unset. The SNMP listener remains unbound, and every notification path, including a test-send button, remains disabled. Report `identity_required` and show: **SNMP disabled: configure a system object ID**. Do not make this expected setup condition fail application health checks.

The administrator enters the **full numeric OID** in **System object ID (`sysObjectID`)**, saves it, configures credentials and notification targets as needed, and enables SNMP. A PEN alone is not a complete value for this field. No registration certificate or ownership lookup is required. The UI must not silently prefill the development value in a public installation.
