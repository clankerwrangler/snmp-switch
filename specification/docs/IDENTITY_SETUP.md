...
5. Choose **Save configuration** to retain the applied switch settings across reboot or process restart. Unsaved changes are discarded; physical lab clients and connections persist automatically.
...
Fresh OID views include **iso** (`1`), **internet** (`1.3.6.1`), and **Identity and interfaces**. The iso view includes the IEEE subtree; internet does not. Existing saved views and assignments remain unchanged. To broaden an existing restricted view, explicitly edit its prefixes. View prefixes accept single ASN.1 roots `0`, `1`, or `2`; the advertised system object ID still requires a complete OID.

Input is parsed as an SNMP-compatible numeric OBJECT IDENTIFIER. Any syntactically valid numeric OID is allowed for testing, including values outside the enterprises prefix. A blank form field applies configuration null; malformed nonempty input is rejected without replacing an existing valid value. The API uses `identity.sys_object_id: null` to clear it explicitly.
...
Clearing the identity stops the listener and sender safely before the clear is acknowledged; it does not stop the simulation. Unsent notification work is discarded with a reason. Events produced while disabled remain local history and are not replayed as traps on activation. A malformed identity in legacy configuration is disabled with a repairable setup warning. A corrupt or incomplete stored startup/lab configuration fails initialization rather than enabling permissive defaults.
