#!/usr/bin/env python3
"""Generate the first-release SNMP object manifest. This is not an SNMP agent."""
...
def add(module,name,oid,syntax,idx,source,formal='outside-core',access='read-only'):
    refs={'SNMPv2-MIB':'RFC3418','IF-MIB':'RFC2863','BRIDGE-MIB':'RFC4188','Q-BRIDGE-MIB':'RFC4363'}
    rows.append(dict(zip(FIELDS,(module,name,oid,syntax,access,'not-readable-index' if access=='not-accessible' else 'read-only',idx,source,formal,refs[module]))))
...
for col,name,syntax,source,formal,access in [
 ...
 (7,'ifAdminStatus','INTEGER','up(1) or down(2)','IfTable','read-write'),
...
for col,name,syntax,source,access in [
 ...
 (4,'dot1qVlanCurrentEgressPorts','PortList','All configured memberships; do not remove down ports','read-only'),
 (5,'dot1qVlanCurrentUntaggedPorts','PortList','Ports whose PVID equals this VLAN','read-only'),
...
for col,name,syntax,source in [
 ...
 (3,'dot1qVlanForbiddenEgressPorts','PortList','All-zero bitmap: no forbidden-membership setting'),
 (4,'dot1qVlanStaticUntaggedPorts','PortList','Ports whose PVID equals this VLAN'),
 (5,'dot1qVlanStaticRowStatus','RowStatus','active(1)'),
...
for col,name,syntax,source,formal in [
 (1,'dot1qPvid','VlanIndex','Configured native VLAN/PVID','Dot1qPvid'),
