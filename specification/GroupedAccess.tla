...
\* Stock PySNMP normal management supports the configured protection level.
\* Authentication remains opaque; discovery/REPORT exceptions are not normal
\* management grants. This is a security-model acceptance predicate, not an
\* additional application floor. The application then checks the group minimum.
StockUsmAccepted(c,level,authenticated) ==
    /\ authenticated
    /\ (cfg.users[c].version = 2 \/ level = cfg.users[c].profile)
RequestBoundary(c,level,source,authenticated,use) ==
    /\ c \in DOMAIN cfg.users /\ cfg.users[c].on
    /\ StockUsmAccepted(c,level,authenticated)
    /\ (cfg.users[c].version = 2 \/ level >= Minimum(cfg,c))
