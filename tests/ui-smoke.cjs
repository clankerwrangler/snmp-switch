...
(async () => {
  ...
  assert.equal('writing' in result.payload,false);assert.deepEqual(result.credential.writing,{enabled:false,view_id:null,networks:[]});assert.equal('polling' in result.payload,false);assert.deepEqual(result.credential.polling,{enabled:false,view_id:'all',networks:[]});assert.ok(result.credential.has_community);
  ...
  assert.deepEqual(snapshot.credentials[created.credential_id].writing,{enabled:false,view_id:null,networks:[]});
  ...
  // SET access uses a separate secret-free form and an explicit selected view.
  const beforeWriting=await state();
  assert.deepEqual(beforeWriting.credentials[noView.credential_id].writing,{enabled:false,view_id:null,networks:[]});
  const openWriting=async()=>{
    await credentialRow(literalLabel).getByRole('button',{name:'SET access',exact:true}).click();
    assert.equal(await page.locator('dialog h2').textContent(),'SET access · '+literalLabel);
    assert.equal(await page.locator('dialog h2 *').count(),0);
    assert.equal(await page.locator('dialog [name=community],dialog [name=username],dialog [name=auth_key],dialog [name=priv_key]').count(),0);
  };
  const saveWriting=async()=>{
    const request=page.waitForRequest(r=>r.url().endsWith('/api/v1/snmp/credentials/'+noView.credential_id)&&r.method()==='PUT');
    await page.getByRole('button',{name:'Save changes',exact:true}).click();
    const payload=(await request).postDataJSON();
    assert.deepEqual(Object.keys(payload).sort(),['expected_configuration_revision','writing']);
    await page.locator('dialog').waitFor({state:'hidden'});
    return (await state()).credentials[noView.credential_id].writing;
  };
  await openWriting();
  assert.equal(await page.getByLabel('SET view').inputValue(),'');
  assert.equal(await page.locator('dialog option:checked').textContent(),'Select a view');
  await page.getByLabel('Allow SET',{exact:true}).check();
  response=page.waitForResponse(r=>r.url().endsWith('/api/v1/snmp/credentials/'+noView.credential_id)&&r.request().method()==='PUT');
  await page.getByRole('button',{name:'Save changes',exact:true}).click();
  assert.equal((await response).status(),422);
  await page.locator('.form-error').filter({hasText:'Select an existing SET view'}).waitFor();
  assert.deepEqual((await state()).credentials,beforeWriting.credentials);
  await page.getByRole('button',{name:'Cancel',exact:true}).click();
  await page.getByRole('button',{name:'+ View',exact:true}).click();
  await page.getByRole('heading',{name:'Add OID view',exact:true}).waitFor();
  await page.getByLabel('Name',{exact:true}).fill('Browser SET view');
  await page.getByLabel('Included numeric OID prefixes (comma-separated)').fill('1.3.6.1.2.1.2.2.1.7');
  response=page.waitForResponse(r=>r.url().endsWith('/api/v1/snmp/views')&&r.request().method()==='POST');
  await page.getByRole('button',{name:'Save changes',exact:true}).click();
  const writeViewId=(await (await response).json()).id;
  await page.locator('dialog').waitFor({state:'hidden'});
  await openWriting();
  assert.equal(await page.getByLabel('SET view').inputValue(),'');
  await page.getByLabel('SET view').selectOption(writeViewId);
  await page.getByLabel('Allow SET',{exact:true}).check();
  await page.getByLabel('Allowed source networks (optional)').fill('192.0.2.0/24, 2001:db8::/32');
  assert.deepEqual(await saveWriting(),{enabled:true,view_id:writeViewId,networks:['192.0.2.0/24','2001:db8::/32']});
  snapshot=await state();
  assert.deepEqual(snapshot.credentials[noView.credential_id].polling,beforeWriting.credentials[noView.credential_id].polling);
  assert.deepEqual(snapshot.targets,beforeWriting.targets);
  await credentialRow(literalLabel).getByRole('button',{name:'Edit',exact:true}).click();
  await page.getByText('SET access: enabled.',{exact:false}).waitFor();
  assert.equal(await page.getByLabel('v2c community',{exact:true}).inputValue(),'');
  await page.getByRole('button',{name:'Cancel',exact:true}).click();
  await openWriting();
  await page.getByLabel('Allowed source networks (optional)').fill('');
  assert.deepEqual(await saveWriting(),{enabled:true,view_id:writeViewId,networks:[]});
  await page.locator(`button[data-action="delete-view"][data-id="${writeViewId}"]`).click();
  response=page.waitForResponse(r=>r.url().endsWith('/api/v1/snmp/views/'+writeViewId)&&r.request().method()==='DELETE');
  await page.getByRole('button',{name:'Delete record',exact:true}).click();
  assert.equal((await response).status(),422);
  await page.locator('.form-error').filter({hasText:'Select an existing SET view'}).waitFor();
  await page.getByRole('button',{name:'Cancel',exact:true}).click();
  await openWriting();
  await page.getByLabel('Allow SET',{exact:true}).uncheck();
  assert.deepEqual(await saveWriting(),{enabled:false,view_id:writeViewId,networks:[]});
  await page.locator(`button[data-action="delete-view"][data-id="${writeViewId}"]`).click();
  await page.getByRole('button',{name:'Delete record',exact:true}).click();
  await page.locator('dialog').waitFor({state:'hidden'});
  await openWriting();
  assert.equal(await page.getByLabel('SET view').inputValue(),writeViewId);
  assert.equal(await page.locator('dialog option:checked').textContent(),'Missing view · '+writeViewId);
  await page.getByRole('button',{name:'Cancel',exact:true}).click();
  assert.deepEqual((await state()).targets,beforeWriting.targets);
  console.log('SET access browser passed: default-off/null view, literal title, no secrets, atomic missing-view failure, explicit independent grant/CIDRs, consumer display, active reference guard and inactive missing-view retention.');
